...
import { CreateHostingSlotRequest, ApiResponse, HostingSlot } from '../types';
...
export const createHostingSlot = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (!slotData.host_name || !slotData.host_address || !slotData.hosting_date) {
      const response: ApiResponse = {
        success: false,
        error: 'Host name, address, and date are required'
      };
